...
function setup() {
    ...
    var button = createButton("Neu anfangen");
    button.mousePressed(resetColors);

    var button_save = createButton("Bild speichern");
    button_save.mousePressed(saveArtwork);
}
...
function draw() {
    ...
    for (var j = 0; j < tileSz * tileNr; j = j + tileSz) {
        for (var k = 0; k < tileSz * tileNr; k = k + tileSz) {
            stroke(200);
            line(j, 0, j, w);
            line(0, k, w, k);
        }
